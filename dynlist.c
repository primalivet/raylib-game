...
void *list_get(dynlist *list, size_t index)
{
  ...
  {
    PRINT_RETURN(NULL, "Falied to retrive item in dynlist as index is out of bounds\n");
  }
...
void *list_remove(dynlist *list, size_t index)
{
  if (list->length == 0) 
  {
    PRINT_RETURN(NULL, "Falied to remove item in dynlist as it is empty\n");
  }
  if (index >= list->length) 
  {
    PRINT_RETURN(NULL, "Falied to remove item in dynlist as index is out of bounds\n");
  }
